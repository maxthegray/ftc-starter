...
8. Verify teleop fault containment on the robot once: in a throwaway teleop, bind a button to a command whose `setExecute` throws. Pressing it should print a `command faults` line in Health telemetry while the drive keeps responding. If the op-mode dies instead, the scheduler's fault containment regressed — fix before competing.
